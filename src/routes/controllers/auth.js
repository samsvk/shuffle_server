...
import { enc, dec, encodeFromData, shuffle, randomInteger } from "../../utils.js";
...
const getUserPlaylistTracks = async (req, res) => {
  ...
  const playlistTracks = await Promise.all(
    playlists.map(async (playlist) => {
      return await fetchPlaylistTracks(dec_at, playlist.id, playlist.length);
    })
  );
...
const createUserPlaylist = async (req, res) => {
  ...
  await fetch(`https://api.spotify.com/v1/users/${req.body.id}/playlists`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${dec_at}`,
      Accept: "application/json",
    ...
    .then((response) => response.json())
    .then(async (data) => {
      const songs = req.body.playlistTunes.map((playlist) => playlist.uri).join(",");
...
async function addSongToPlaylist(dec_at, id, songs) {
  await fetch(`	https://api.spotify.com/v1/playlists/${id}/tracks?uris=${songs}`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${dec_at}`,
      Accept: "application/json",
